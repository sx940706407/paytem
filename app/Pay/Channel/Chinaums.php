...
class Chinaums extends BasePayData
{
    /**
    ...
    {
        $chinaumsPayConfig = Config::get('channelpay.chinaums');
        $orderNo = $chinaumsPayConfig['msgSrcId'].'CHINAUMSPAY'.date('Ymd').rand(1000, 9000);
        $payData  = [
            'mid'    => $chinaumsPayConfig['mid'], //商户编号
            'msgSrc'    => $chinaumsPayConfig['msgSrc'],
            'tid'   => $chinaumsPayConfig['tid'],
            'totalAmount'    => $request['totalfee'] * 100, //默认是按元传进来  * 100
            'notifyUrl'    => 'http://notify.dfylpro.com:10003/pay/chinaumspay/notifyUrl',
            'returnUrl'   => 'http://notify.dfylpro.com:10003/pay/chinaumspay/notifyUrl/success',
            'msgId'    => $request['body'],
            'merOrderId'    => $orderNo,
            'requestTimestamp'  => date('Y-m-d H:m:s'),
            'instMid'   => $chinaumsPayConfig['instMid'],
            'signType'  => 'MD5',
        ];
        $payData = $this->paymentData($request, $payData);
        try {
            $ret = Charge::run(PayConfig::CHINAUMS_CHANNEL, $chinaumsPayConfig, $payData);
            $packageReturnChinaums = $this->packageReturnChinaums($ret, $request, $orderNo);

            if (isset($packageReturnChinaums['resultUrl'])) {
                $shopId = ShopModels::where('app_id', $request['app_id'])->value('id');

                Log::info('Channel\CHINAUMS\handle====>STATUS===>T', ['ret'=>$ret]);
                $data = [
                    'third'	=> 'chinaums',
                    'channel'	=> $request['paymethod'],
                    'payType'	=> $request['call'],
                    'body'	=> $request['body'] ?? 'chinaums支付操作',
                    'subject'	=> $request['body'] ?? 'chinaums支付操作',
                    ...
                    'pay_callback_content'	=>$request['paymethod'] == 'unionpay' ?
                                                                    $ret :
                                                                    json_encode($ret),
                ...
                if ($request['paymethod'] == 'unionpay') {
                    $packageReturnChinaums['response']['stateData']['mweburl'] = url('chinaums/'.$PayInfoModels->id);
                }
                return $packageReturnChinaums['response'];
            }
            Log::info('Channel\CHINAUMS\handle===>STATUS===>F', ['ret'  => $ret]);
            return $this->ConfigRetrun(1003);
        } catch (PayException $e) {
            Log::info('Channel\CHINAUMS\handle===>STATUS===>PayException', ['e'  => $e]);
            return  $this->ConfigRetrun(1003);
            exit;
        }
    }
