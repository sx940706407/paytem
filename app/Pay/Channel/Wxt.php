...
class Wxt
{
    ...
    public function handle($request)
    {
        ...
        try {
            ...
            if ($ret['status'] == 'T') {
                ...
                Log::info('Channel\WXT\handle========>STATUS==================>T', ['response'=> $response,'ret'  => $ret]);
                $data = [
                    'third'	=> 'zn',
            ...
            Log::info('Channel\WXT\handle========>STATUS==================>F', ['response'=> $response,'ret'  => $ret]);
    ...
    public function handleStatus($request)
    {
        ...
        try {
            ...
            #支付状态status T 返回成功处理
            if ($ret['status'] == 'T') {
                Log::info('Channel\WXT\handleStatus========>status==========>T', ['response'=> $response,'ret'  => $ret]);
            ...
            Log::info('Channel\WXT\handleStatus========>status==========>F', ['response'=> $response,'ret'  => $ret]);
    ...
    public function handleBank($request)
    {
        ...
        try {
            ...
            if ($ret['status'] == 'T') {
                //记录到代付表中或者放在
                Log::info('Channel\WXT\handleBank=========>status=========>T', ['response'=> $response,'ret'  => $ret,'request'=> $request]);

                $shopId = ShopModels::where('app_id', $request['app_id'])->first();
                $data = [
                    'third' => 'zn',
                    'channel' => 'bank',
                    'payType' => 'payment',
                ...
                $shopMoney = [
                    ...
                    'after_money'   => $shopId->moeny - ($request['trans_money'] - 3) * 100 ,
                    'active'    => '兑换钱数:(分为单位)'.($request['trans_money'] - 3) * 100  . '||兑换订单'.$order_id,
                ];
                ShopMoney::create($shopMoney);
                //减去商户的钱数，增加与减少都需要对应的操作记录
                $shopId->moeny = $shopId->moeny - $request['trans_money'] * 100 ;
                $shopId->save();
            ...
            Log::info('Channel\WXT\handleBank=========>status=========>F', ['response'=> $response,'ret'  => $ret,'request'  => $request]);
            return Config::get('error.1004');
        } catch (PayException $e) {
            return  Config::get('error.1004');
            exit;
    ...
    public function handleBankStatus($request)
    {
        ...
        try {
            ...
            #支付状态status T 返回成功处理 
            if ($ret['status'] == 'T') {
                Log::info('Channel\WXT\handleBankStatus=========>status=========>T', ['response'=> $response,'ret'  => $ret]);
                $response['stateData']['tradestate'] = 'TRADE_SUCCESS';
                $response['stateData']['orderid'] = $orderNo;
...
//{"errMsg":"受理中","merchantNo":"32413110017685","orderNo":"ZHONG20180908191536406713","serialNo":"200000004521875","sign":"bef38ce42164f87edbbde23a4252f9c9","status":"T","succeedAmount":"0","transStatus":"1"}
            ...
            Log::info('Channel\WXT\handleBankStatus=========>status=========>F', ['response'=> $response,'ret'  => $ret]);
            return Config::get('error.1004');
        } catch (PayException $e) {
            return  Config::get('error.1004');
            exit;
